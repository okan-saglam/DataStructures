...
class MeshGraph
{
    ...
    void    Dijsktra(std::vector<int>&path, int vertexfrom, int vertexto) const;
    void    BFS(std::vector<int>& toBeColored, std::vector<int>& previous, int vertexId, int maxDepth) const;
    double  Gaussian(double x, double alpha) const;
    int     Box(double x, double alpha) const;
